...
test.describe('Visual Regression Tests', () => {
  ...
  test('Profile image visual consistency', async ({ page }) => {
    ...
    const profileImage = page.locator('img[alt*="profile"], img[alt*="Rafael"], .profile-image').first();
    if (await profileImage.count() > 0) {
      await expect(profileImage).toHaveScreenshot('profile-image-baseline.png', {
        threshold: 0.1
      });
    }
  });
  ...
  test('Cross-browser visual consistency - Chrome vs Firefox', async ({ page, browserName }) => {
