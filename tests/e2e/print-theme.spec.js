...
test.describe('Print Theme Functionality', () => {
  ...
  test('should force light theme variables when print media is detected', async ({ page }) => {
    ...
    if (initialTheme === 'dark') {
      const darkBackground = await page.evaluate(() => {
        return getComputedStyle(document.documentElement).getPropertyValue('--color-background-base');
      });
    ...
    const printAccentColor = await page.evaluate(() => {
      return getComputedStyle(document.documentElement).getPropertyValue('--color-accent-primary');
    });
  ...
  test('should maintain light theme when already in light mode during print', async ({ page }) => {
  ...
  test('should return to original theme after print preview exits', async ({ page }) => {
    // Set dark theme
    await page.evaluate(() => {
  ...
  test('should force light theme CSS variables with !important in print media query', async ({ page }) => {
          ...
          for (const rule of cssRules) {
            if (rule.type === CSSRule.MEDIA_RULE && rule.media.mediaText.includes('print')) {
              ...
              for (const cssRule of mediaRules) {
                if (cssRule.selectorText === ':root' && cssRule.cssText.includes('!important')) {
    ...
    expect(printRuleText).toContain('--color-background-base: #ffffff !important');
    expect(printRuleText).toContain('--color-text-primary: #000000 !important'); 
    expect(printRuleText).toContain('--color-accent-primary: #2196f3 !important');
  });
});