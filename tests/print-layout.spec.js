...
test.describe('Print Layout Comprehensive Tests', () => {
  ...
  test.describe('Print Content Visibility', () => {
    ...
    test('Print - Main content sections are fully visible', async ({ page }) => {
    ...
    test('Print - Education and projects sections visible', async ({ page }) => {
      // Education items
  ...
  test.describe('Print Element Hiding', () => {
    ...
    test('Print - Interactive elements are properly hidden', async ({ page }) => {
    ...
    test('Print - Social media links are hidden for clean look', async ({ page }) => {
  ...
  test.describe('Print Typography and Styling', () => {
    ...
    test('Print - Body has correct background and text colors', async ({ page }) => {
    ...
    test('Print - Content sections have high contrast', async ({ page }) => {
      ...
      expect(['rgba(0, 0, 0, 0)', 'transparent', 'rgb(255, 255, 255)']).toContain(bgColor);
    });

    test('Print - Typography uses serif font for readability', async ({ page }) => {
      const body = page.locator('body');
    ...
    test('Print - Font size is optimized for print (11pt)', async ({ page }) => {
      const body = page.locator('body');
    ...
    test('Print - No shadows or visual effects', async ({ page }) => {
      ...
      const elementsToCheck = ['.work-item', '.section', '.project-item', 'h1', 'h2'];
      ...
      for (const selector of elementsToCheck) {
        ...
        if (count > 0) {
          const boxShadow = await elements.first().evaluate(el => 
            window.getComputedStyle(el).boxShadow
          );
          const textShadow = await elements.first().evaluate(el => 
  ...
  test.describe('Print Layout Structure', () => {
    ...
    test('Print - Main content uses block layout', async ({ page }) => {
      ...
      if (await mainContent.count() > 0) {
        const display = await mainContent.evaluate(el => 
  ...
  test.describe('Print Page Breaking', () => {
    ...
    test('Print - Work items avoid page breaks inside', async ({ page }) => {
      ...
      for (let i = 0; i < Math.min(workCount, 3); i++) {
        ...
        const pageBreakInside = await workItem.evaluate(el => 
    ...
    test('Print - Section headers stay with content', async ({ page }) => {
      ...
      for (let i = 0; i < Math.min(headerCount, 3); i++) {
        ...
        const pageBreakAfter = await header.evaluate(el => 
    ...
    test('Print - Orphans and widows are controlled', async ({ page }) => {
      ...
      if (pCount > 0) {
        ...
        const orphans = await firstP.evaluate(el => 
          window.getComputedStyle(el).orphans
        );
        const widows = await firstP.evaluate(el => 
          window.getComputedStyle(el).widows
        );
  ...
  test.describe('Print Content Integrity', () => {
    ...
    test('Print - Contact information is readable', async ({ page }) => {
      ...
      if (await contactInfo.count() > 0) {
        ...
        const hasEmail = contactText.includes('@') || contactText.includes('email');
        const hasPhone = /\d{3}/.test(contactText) || contactText.includes('phone');
        const hasLocation = contactText.includes('Brazil') || contactText.includes('São Paulo');
    ...
    test('Print - Skills and technologies are visible', async ({ page }) => {
      ...
      const skillsSection = page.locator('.skills, .technologies, [class*="skill"]');
    ...
    test('Print - Work experience dates and details preserved', async ({ page }) => {
      const workItems = page.locator('.work-item');
      const workCount = await workItems.count();
      ...
      if (workCount > 0) {
        ...
        const hasDatePattern = /\d{4}|\d{1,2}\/\d{4}|present|current/i.test(workText);
