...
function getQRCodeURL() {
  try {
    ...
    if (process.env.NETLIFY === 'true') {
      ...
      if (prNumber) {
        return `https://deploy-preview-${prNumber}--magnificent-chimera-c0fbb4.netlify.app`;
      }
      // Netlify branch deploy or production
      const branchName = process.env.HEAD || process.env.BRANCH;
      if (branchName && branchName !== 'main') {
        return `https://${branchName}--magnificent-chimera-c0fbb4.netlify.app`;
      }
      // Netlify production
      return 'https://magnificent-chimera-c0fbb4.netlify.app';
    }
