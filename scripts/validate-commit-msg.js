...
const PATTERNS = {
  ...
  breaking: /^(feat|fix|docs|style|refactor|perf|test|build|ci|chore|revert)(\(.+\))?(!):/,
  description: /.{1,50}$/
};
...
function validateCommitMessage(message) {
  ...
  if (!CONVENTIONAL_COMMIT_PATTERN.test(firstLine)) {
    return {
      ...
      details: [
        ...
        '',
        'Valid types:',
        ...Object.entries(TYPE_DESCRIPTIONS).map(([type, desc]) => `  ${type.padEnd(8)} - ${desc}`)
      ]
    };
  }
  ...
  if (description.length > 50) {
    console.log(`⚠️  Description is ${description.length} chars. Consider keeping the subject line under 50 chars for better readability.`);
    console.log('💡 Long descriptions are fine for detailed commits - GitHub will wrap them appropriately.');
  }
  
  // Check if description starts with lowercase (conventional style)
  if (description[0] !== description[0].toLowerCase()) {
    console.log('⚠️  Description should start with lowercase letter (conventional style)');
  }
  
  // Validate that description doesn't end with period
  if (description.endsWith('.')) {
    console.log('⚠️  Description should not end with a period (conventional style)');
  }
  
  console.log('✅ Commit message follows Conventional Commits format');
  
  if (breaking) {
    console.log('🚨 BREAKING CHANGE detected - this will trigger a major version bump');
  }
  ...
  if (['fix', 'perf'].includes(type)) {
    console.log('🔧 Bug fix/performance improvement - this will trigger a patch version bump');
  }
...
function processMessage(message) {
  ...
  if (result.valid) {
    ...
    console.error('');
    console.error('📚 Learn more about Conventional Commits: https://www.conventionalcommits.org/');
    process.exit(1);
  }
}
