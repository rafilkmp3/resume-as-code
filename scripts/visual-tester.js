...
test('${testCase.description}', async ({ page }) => {
    ...
    const recommendations = [
      {
        ...
        priority: 'High',
        description:
          'Analyze iPhone 16 Pro Max screenshots for touch target sizes, text readability, and navigation usability',
        evidence: 'iphone-16-pro-max screenshots',
        action:
          'Review mobile layout, ensure 44px+ touch targets, optimize font sizes',
      },
      {
        category: 'Desktop Experience',
        priority: 'High',
        description:
          'Evaluate macOS Retina screenshots for layout efficiency, whitespace usage, and content hierarchy',
        evidence: 'macos-retina screenshots',
        action:
          'Optimize desktop layout, improve content density, enhance visual hierarchy',
      },
      {
        category: 'Theme Consistency',
        priority: 'Medium',
        description:
          'Compare dark vs light mode screenshots for consistency and contrast',
        evidence: 'light vs dark mode comparisons',
        action:
          'Ensure consistent spacing, improve color contrast, fix theme-specific issues',
      },
      {
        category: 'Visual Elements',
        priority: 'Medium',
        description:
          'Assess buttons, links, and interactive elements across all devices',
        evidence: 'all device screenshots',
        action:
          'Enhance button visibility, improve hover states, optimize interactive feedback',
      },
      {
        ...
        action:
          'Improve content grouping, enhance visual separators, optimize reading flow',
      },
    ];
  ...
  generateReport() {
    ...
    console.log('\n📸 SCREENSHOT SUMMARY:');
    console.log(
      `   Total Screenshots: ${this.testResults.summary.total_screenshots}`
    );
    console.log(
      `   Devices Tested: ${this.testResults.summary.devices_tested}`
    );
    ...
    if (this.testResults.summary.issues_found.length > 0) {
      ...
      this.testResults.summary.issues_found.forEach((issue, index) => {
        console.log(
          `   ${index + 1}. ${issue.type} - ${issue.device} (${issue.theme})`
        );
      });
    } else {
    ...
    Object.entries(this.testResults.devices).forEach(([device, themes]) => {
      ...
      Object.entries(themes).forEach(([theme, result]) => {
        console.log(
          `     ${this.themes[theme].name}: ${result.status} - ${result.screenshot}`
        );
      });
    });
