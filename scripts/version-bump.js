...
function analyzeCommits() {
  try {
    ...
    try {
      ...
      console.log(
        '📝 No previous tags found, starting from commits since repository creation'
      );
      lastTag = null;
    }
    ...
    console.log(
      `📊 Analyzing ${commits.length} commits since ${lastTag || 'repository creation'}:`
    );
    ...
    const conventionalPatterns = {
      ...
      patch:
        /^[a-f0-9]+\s+(fix|bug|patch|perf|refactor|docs|test|ci|build|chore)(\(.+\))?:/i,
    };
...
`;
  ...
  for (const commit of analysis.commits) {
    ...
    if (/^(.+!):|BREAKING\s+CHANGE/i.test(message)) {
      ...
      /^(perf|refactor|docs|test|ci|build|chore)(\(.+\))?:/i.test(message)
    ) {
...
function main() {
  ...
  console.log(
    `🚀 Version bump: ${oldVersionString} → ${newVersionString} (${analysis.type.toUpperCase()})`
  );
  ...
  try {
    ...
    execSync('node scripts/update-version-refs.js');
    console.log('✅ Version references updated');
  } catch (error) {
    console.error('⚠️  Failed to update version references:', error.message);
  }

  // Stage the changes
  try {
    execSync('git add package.json CHANGELOG.md docs/ scripts/');
    console.log('📝 Staged version bump and documentation changes');
  } catch (error) {
...
Co-Authored-By: Claude <noreply@anthropic.com>`;
  ...
  try {
    ...
    console.error(
      '⚠️  Failed to fetch tags, continuing anyway:',
      error.message
    );
  }
